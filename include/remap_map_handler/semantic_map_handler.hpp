...
public:
    ...
    const openvdb::Vec3d & origin = openvdb::Vec3d(0.0, 0.0, 0.0))
  {
    if (length <= 0.0) {
      std::cerr <<
        "[AddPyramid]: The cone length cannot be less than or equal to zero! Aborting!" <<
        std::endl;
    } else {
    ...
    auto kernel = [&](const tbb::blocked_range2d<int> & iteration_range)
      {
        ...
        for (int row = iteration_range.rows().begin(); row != iteration_range.rows().end(); ++row) {
          for (int col = iteration_range.cols().begin(); col != iteration_range.cols().end();
            ++col)
          {
            ...
            if (std::isnan(x_metric) == false && std::isnan(y_metric) == false &&
              std::isnan(z_metric) == false)
            {
              openvdb::Vec3d coordinates =
                grid_->worldToIndex(openvdb::Vec3d(x_metric, y_metric, z_metric));
              openvdb::Coord ijk(static_cast<int>(round(coordinates[0])),
                static_cast<int>(round(coordinates[1])), static_cast<int>(round(coordinates[2])));
